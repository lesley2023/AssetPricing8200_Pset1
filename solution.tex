...
\subsection{Part (a)}
I construct the monthly momentum signal for each stock using CRSP returns. Momentum at the end of month $\tau$ is the cumulative return over the preceding 12 months, excluding the current month:

\[ MOM_{i,\tau} =\prod_{k=1}^{12}(1+RET_{i,\tau-k})-1 \].

I require 12 valid monthly returns in consecutive calendar months. The sample begins in June 1963 and is restricted to common stocks with share codes 10 or 11 listed on NYSE, Amex or Nasdaq. I exclude utilities and financial firms.

To validate the constructed momentum signal, I merge the CRSP observations with the Chen$\textendash$Zimmermann dataset. I then estimate the following cross-sectional regression separately for each month:

\[ MOMCZ_{i,\tau} =a_\tau+b_\tau MOM_{i,\tau} +\varepsilon_{i,\tau}. \]

Figures \ref{fig:q3a_intercepts}–\ref{fig:q3a_r2} plot the monthly intercept estimates, slope estimates and $(R^2)$ values, respectively.

The estimated intercepts generally fluctuate around zero. The slopes generally remain close to one. The $(R^2)$ values are consistently high and close to one. The results suggest that the CRSP-based momentum signal closely matches the Chen–Zimmermann measure.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3a_intercepts.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3a_slopes.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3a_r2.png}
    \caption{Monthly cross-sectional regression $R^2$ values.}
    \label{fig:q3a_r2}
\end{figure}
\FloatBarrier
\clearpage
...
\subsection{Part (b)}
I construct the book-to-market ratio for each stock using CRSP and Compustat data. For the portfolio formed at the end of June of year $t$, I calculate

\[
BM_{i,t}=\frac{BE_{i,t-1}}{ME_{i,\mathrm{Dec}(t-1)}},
\]

where market equity is calculated as $ME=|PRC|\times SHROUT$, and book equity is calculated as $BE=SE+TXDITC-BVPS$, using the specified Compustat variable hierarchy. The resulting BM signal is held constant from June of year $t$ through May of year $t+1$.

I require at least two previous years of Compustat records and positive book equity. I restrict the sample to common stocks listed on NYSE, Amex, or Nasdaq, excluding utilities and financial firms. The sample begins in June 1963.

To compare my constructed BM signal with the Chen–Zimmermann measure, I merge the two datasets by stock and month and estimate the following cross-sectional regression separately for each month:

\[
BM_{CZ,i,\tau}=a_\tau+b_\tau BM_{i,\tau}+\varepsilon_{i,\tau},
\]

Figures \ref{fig:q3b_intercepts}–\ref{fig:q3b_r2} report the monthly intercepts, slopes, and $R^2$ values, respectively. The results indicate that the constructed BM signal closely matches the Chen–Zimmermann measure. Across the sample period except for June 2024, the intercepts are generally close to zero, the slopes are generally close to one, and the regressions exhibit high explanatory power. 

As a robustness check, I investigate the sharp decline in the regression statistics observed in June 2024. Further examination identifies a highly influential observation, PERMNO 18558, which has exceptionally large book-to-market values and therefore substantial leverage in the cross-sectional regression. To assess whether the results are driven by this extreme observation, I winsorize both my constructed BM measure and the Chen–Zimmermann BM measure separately at the 1st and 99th percentiles within each month and re-estimate the monthly regressions. After winsorization, from the Figure \ref{fig:q3b_winsorized_intercepts}–\ref{fig:q3b_winsorized_r2}, the results suggest that the two BM measures remain closely aligned after reducing the influence of extreme observations.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3b_intercepts.png}
    \caption{Monthly cross-sectional regression intercepts.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3b_slopes.png}
    \caption{Monthly cross-sectional regression slopes.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3b_r2.png}
    \caption{Monthly cross-sectional regression $R^2$ values.}
    \label{fig:q3b_r2}
\end{figure}
\FloatBarrier

\paragraph{1\% winsorized robustness check.}
The full-sample June 2024 regression is dominated by PERMNO 18558, whose exceptionally large book-to-market values give it very high cross-sectional leverage. As a sensitivity analysis, I winsorize both constructed $BM$ and $BM_{CZ}$ separately within each month at their 1st and 99th percentiles before estimating the same monthly regressions. These robustness results do not replace the full-sample estimates above.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3b_winsorized_intercepts.png}
    \caption{Monthly cross-sectional regression intercepts after 1\% winsorization of both BM measures.}
    \label{fig:q3b_winsorized_intercepts}
\end{figure}
\FloatBarrier

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3b_winsorized_slopes.png}
    \caption{Monthly cross-sectional regression slopes after 1\% winsorization of both BM measures.}
    \label{fig:q3b_winsorized_slopes}
\end{figure}
\FloatBarrier

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q3b_winsorized_r2.png}
    \caption{Monthly cross-sectional regression $R^2$ values after 1\% winsorization of both BM measures.}
    \label{fig:q3b_winsorized_r2}
\end{figure}
\FloatBarrier
\clearpage

\subsection{Part (c)}
Figures \ref{fig:q3c_vw_annual_nyse}–\ref{fig:q3c_ew_monthly_general} present the average monthly excess returns of decile portfolios formed using the three Chen–Zimmermann signals: book-to-market (BM), momentum (MOM), and gross profitability (GP). The five portfolio-construction methods differ in their weighting schemes, rebalancing frequencies, and breakpoint definitions. Table \ref{tab:q3c_hml} reports the corresponding high-minus-low (HML) average monthly returns and Newey–West t-statistics.

Several patterns emerge from the figures. First, BM generally exhibits a positive relationship between signal rankings and average excess returns. This pattern is particularly pronounced in the equal-weighted portfolios (Figures \ref{fig:q3c_ew_annual_nyse} and \ref{fig:q3c_ew_monthly_general}), whereas the differences between low- and high-BM deciles are smaller in the value-weighted portfolios (Figures \ref{fig:q3c_vw_annual_nyse}, \ref{fig:q3c_vw_monthly_nyse}, and \ref{fig:q3c_vw_annual_general}).

Second, momentum exhibits greater sensitivity to portfolio-construction methods. The return spread between high- and low-momentum deciles is particularly pronounced under monthly value-weighted rebalancing with NYSE breakpoints (Figure \ref{fig:q3c_vw_monthly_nyse}). In contrast, the relationship is weaker under annual rebalancing. In the equal-weighted, annually rebalanced portfolios with NYSE breakpoints (Figure \ref{fig:q3c_ew_annual_nyse}), the highest-momentum decile has a slightly lower average excess return than the lowest-momentum decile.

Third, GP generally exhibits higher average excess returns in the high-signal deciles than in the low-signal deciles across all five portfolio-construction methods. However, the differences are relatively modest in the value-weighted portfolios and more pronounced in the equal-weighted portfolios.

Comparing the five figures, portfolio weighting and rebalancing frequency appear to have substantial effects on the observed return patterns. These patterns are also reflected in Table \ref{tab:q3c_hml}. BM and GP have positive HML returns across all five methods, whereas the magnitude and sign of the momentum HML return vary with portfolio construction. 
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/q3c_vw_annual_nyse.png}
    \caption{Average monthly excess returns: value-weighted annual portfolios with NYSE breakpoints.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/q3c_ew_annual_nyse.png}
    \caption{Average monthly excess returns: equal-weighted annual portfolios with NYSE breakpoints.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/q3c_vw_monthly_nyse.png}
    \caption{Average monthly excess returns: value-weighted monthly portfolios with NYSE breakpoints.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/q3c_vw_annual_general.png}
    \caption{Average monthly excess returns: value-weighted annual portfolios with general breakpoints.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/q3c_ew_monthly_general.png}
    \caption{Average monthly excess returns: equal-weighted monthly portfolios with general breakpoints.}
...
\begin{table}[htbp]
    \centering
    \caption{Average monthly HML returns and Newey--West $t$-statistics}
    \label{tab:q3c_hml}
    \small
    \begin{tabular}{llrr}
        \toprule
        Portfolio construction & Signal & Mean (\%) & NW $t$-statistic \\
        \midrule
        VW annual, NYSE & BMCZ  & 0.3295 & 1.579 \\
        VW annual, NYSE & MOMCZ & 0.4672 & 1.979 \\
        VW annual, NYSE & GPCZ  & 0.3059 & 1.932 \\
        EW annual, NYSE & BMCZ  & 1.0030 & 5.751 \\
        EW annual, NYSE & MOMCZ & $-0.2164$ & $-1.063$ \\
        EW annual, NYSE & GPCZ  & 0.4393 & 2.856 \\
        VW monthly, NYSE & BMCZ  & 0.3087 & 1.494 \\
        VW monthly, NYSE & MOMCZ & 1.1644 & 4.307 \\
        VW monthly, NYSE & GPCZ  & 0.3114 & 1.969 \\
        VW annual, general & BMCZ  & 0.3065 & 1.352 \\
        VW annual, general & MOMCZ & 0.5674 & 2.035 \\
        VW annual, general & GPCZ  & 0.3758 & 1.873 \\
        EW monthly, general & BMCZ  & 0.9496 & 5.266 \\
        EW monthly, general & MOMCZ & 0.6273 & 2.228 \\
        EW monthly, general & GPCZ  & 0.4976 & 2.545 \\
        \bottomrule
    \end{tabular}
\end{table}
\FloatBarrier
...
\begin{table}[htbp]
    \centering
    \caption{Hold-to-maturity excess bond return regressions}
    \label{tab:q4b_bond_regressions}
    \begin{tabular}{lcccc}
        \toprule
        & $H=2$ & $H=3$ & $H=4$ & $H=5$ \\
        \midrule
        $\widehat{b}^{(H)}$ [HH $t$-stat] & 0.6931 [3.4373] & 0.5201 [2.9990] & 0.3935 [2.2761] & 0.3140 [2.0716] \\
        $R^2$ & 8.0851\% & 5.4333\% & 3.7891\% & 2.8126\% \\
        $N$ & 811 & 811 & 811 & 811 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\clearpage
\subsection{Part (c)}
I estimate Equation (4.2) separately for $H=2,3,4,5$ using monthly observations. I match the forward spread observed in month $t$ with the excess annual bond return ending 12 months later. 
The regressions include an intercept, and the reported $t$-stats use Newey and West (1987, 1994) standard errors with an automatically selected lag length.

In Table~\ref{tab:q4c_forward_regressions}, the estimated coefficients are positive and statistically significant at the 5\% level for all four maturities. Thus, higher forward spreads are associated with higher excess bond returns over the following year. 
The predictive relationship is strongest for the four-year bond, which has the largest coefficient and the highest $R^2$, although the coefficient falls somewhat at the five-year maturity.

Compared with the yield-spread regressions in Part (b), the forward-spread regressions have higher $R^2$ values for the three-, four-, and five-year bonds. 
This suggests that forward spreads contain more predictive information about one-year-ahead excess returns at these maturities. For $H=2$, the coefficient and $R^2$ are the same as in Part (b), consistent with the relationship $xf_{b,t}^{(2)}=2xy_{b,t}^{(2)}$.

\begin{table}[htbp]
    \centering
    \caption{One-year excess bond return regressions on forward spreads}
    \label{tab:q4c_forward_regressions}
    \begin{tabular}{lcccc}
        \toprule
        & 2 years & 3 years & 4 years & 5 years \\
        \midrule
        \multicolumn{5}{l}{\textit{Forwards}} \\
        $\widehat{b}^{(H)}$ [NW $t$-stat] & 0.6774 [3.2062] & 0.8887 [3.3257] & 1.1163 [3.5907] & 0.9641 [2.9108] \\
        $R^2$ & 7.6916\% & 8.6265\% & 10.7201\% & 6.6659\% \\
        \bottomrule
    \end{tabular}
\end{table}
...
\clearpage
\subsection{Part (d)}
I estimate the Cochrane–Piazzesi factor by regressing the average one-year-ahead excess return across the two- to five-year bonds on the current one- to five-year forward rates:
$$ \frac{1}{4}\sum_{H=2}^{5}xr_{b,t+1}^{(H)} = \theta_0+\sum_{h=1}^{5}\theta_h f_{b,t}^{(h)}+u_t. $$
Using the estimated slope coefficients, I construct
$$ cp_t=\sum_{h=1}^{5}\widehat{\theta}_h f_{b,t}^{(h)}. $$
Figure~\ref{fig:q4d_cp_factor} plots the estimated factor in percentage terms, with NBER recessions shown by the shaded areas. The CP factor varies considerably over time, suggesting substantial time variation in expected excess bond returns. 
Although the factor sometimes moves sharply around recessions, it does not follow the same pattern in every recession. The factor reaches its highest levels during the first half of the 1980s and remains relatively elevated in the early 1990s and early 2000s. 
It is comparatively low around 2020 and becomes negative during parts of 2022–2024.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/q4d_cp_factor.png}
    \caption{Cochrane--Piazzesi factor with NBER recession periods.}
    \label{fig:q4d_cp_factor}
\end{figure}
...
\begin{table}[htbp]
    \centering
    \caption{One-year excess bond return regressions on the CP factor}
    \label{tab:q4e_cp_regressions}
    \begin{tabular}{lcccc}
        \toprule
        & 2 years & 3 years & 4 years & 5 years \\
        \midrule
        \multicolumn{5}{l}{\textit{CP factor}} \\
        $\widehat{b}^{(H)}$ [NW $t$-stat] & 0.4418 [4.1257] & 0.8274 [4.1243] & 1.2517 [4.3896] & 1.4791 [4.1986] \\
        $R^2$ & 13.6703\% & 14.3774\% & 17.0468\% & 15.5228\% \\
        \bottomrule
    \end{tabular}
\end{table}
