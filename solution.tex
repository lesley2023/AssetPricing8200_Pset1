...
\subsection{Part (b)}
I estimate Equation (4.1) separately for $(H=2,3,4,5)$ using monthly observations. When constructing the hold-to-maturity return, I reduce the maturity of the bond by one year after each 12-month period. The regressions include an intercept, and the reported \(t\)-statistics use Hansen and Hodrick (1980) standard errors with \(L=12H-1\) monthly lags.

In Table~\ref{tab:q4b_bond_regressions}, the estimated coefficients are positive and statistically significant at the 5\% level for all four maturities. This indicates that higher yield spreads are associated with higher subsequent hold-to-maturity excess bond returns. Both the coefficient estimates and the \(R^2\) values decline with maturity, suggesting that the predictive relationship is stronger for shorter-maturity bonds in this sample.
\begin{table}[htbp]
    \centering
    ...
    \begin{tabular}{lcccc}
        ...
        & $H=2$ & $H=3$ & $H=4$ & $H=5$ \\
        \midrule
        $\widehat{b}^{(H)}$ [HH $t$-stat] & 0.6774 [3.5732] & 0.5316 [3.2149] & 0.4141 [2.5196] & 0.3453 [2.3001] \\
        $R^2$ & 7.6916\%  & 5.1512\%  & 3.7705\%  & 3.2346\%  \\
        \bottomrule
    \end{tabular}
\end{table}
