...
For each horizon $H=1,\ldots,15$, I construct the average of the simple annual
excess returns observed exactly 12, 24, $\ldots$, $12H$ months after each starting
month $t$,
\[
Y_t^{(H)}=\frac{1}{H}\sum_{h=1}^{H}xR_{e,t+12h},
\qquad
xR_{e,t}=e^{re_t}-e^{rf_t}.
\]
I then estimate by OLS, including an intercept,
\[
Y_t^{(H)}=a^{(H)}+b^{(H)}e^{dp_t}+\epsilon_t^{(H)}.
\]
Only starting months with all $H$ required future annual observations are used.
...
average excess returns than for one-year-ahead excess returns. The relationship
is not perfectly monotonic at the longest horizon, but the overall pattern is a
strong increase in predictive fit as the horizon lengthens.
\FloatBarrier
